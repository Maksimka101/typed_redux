import 'package:typed_redux/src/store.dart';
...
/// ]);
/// ```
class WhereTypeReducer<State, BaseAction, Action extends BaseAction>
    implements TypedReducerClass<State, BaseAction> {
  ...
  WhereTypeReducer(this.reducer);

  @override
  State call(State state, BaseAction action) {
    if (action is Action) {
...
///
/// final middleware = (TypedStore<AppState> store, action, NextDispatcher next) {
...
/// ];
/// ```
class WhereTypeMiddleware<State, BaseAction, Action>
    implements TypedMiddlewareClass<State, BaseAction> {
  /// A [Middleware] function that only works on actions of a specific type.
  final void Function(
    TypedStore<State, BaseAction> store,
    Action action,
    TypedNextDispatcher<BaseAction> next,
  ) middleware;

  /// Create a [Middleware] that is only executed when the dispatched action
  /// matches the [Action] type.
  WhereTypeMiddleware(this.middleware);
...
///       friendReducer,
///     );
TypedReducer<State, Action> combineReducers<State, Action>(
  Iterable<TypedReducer<State, Action>> reducers,
) {
